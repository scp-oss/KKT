...
func TestNormalizeAddress(t *testing.T) {
	cases := []struct{ in, want string }{
		...
		// "об." (not just "обл.") as the область abbreviation, and "ст р."
		// (a split "стр.") joined back together.
		{
			"Пензенская об.,г. Заречный ул. Зеленая, ст р. 6г",
			"г. Заречный ул. Зеленая, стр. 6г",
		},
		{"", ""},
	}
	for _, c := range cases {
